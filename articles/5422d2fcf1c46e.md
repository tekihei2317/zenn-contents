---
title: "Prismaが実行するSQLを観察してみる（参照系）"
emoji: "🎃"
...
主に参照系で、PrismaがどのようなSQLを実行しているのかが気になったため、調べてみることにしました。ドキュメントをざっくり見た感じでは、実行されるSQLについて記述されているページがなかったため、実際にPrisma Clientを動かして確認してみました。
...
一つのテーブルに対しての操作は、以下のようにPrisma Clientの記述と実行されるSQLがほぼ同じなので、主にリレーションに関する操作を中心に見ていきます。
...
集約系のAPI（`aggregate`と`groupBy`）もありますが、今回は試していません。
...
実行されるSQLは以下です。どちらも同じように、Userを取得した後に、そのIDを使ってPostを取得するクエリを実行していることが分かります。
...
`include`と`select`の違いは、`include`がリレーションにしか使えないのに対し、`select`はフィールドとリレーションの両方に使えることです。また、`include`を使った場合は、親のテーブルの全てのカラムが暗黙的に取得されます。

注意点は、`include`と`select`は同じ階層に書けないことです。この理由は、実行されるSQLを見ても分かるように、`select`と`include`の役割が被っているからだと考えられます。つまり、両方の記述がある場合は、prismaがどちらを優先すればいいのかが分かりません。
...
また、SQLの組み立ては、取得・検索・ソートで独立して行われていそうです。つまり、リレーションでの検索とソートを同時に行う場合は、サブクエリでの検索とJOINの両方が行われます（例は？パフォーマンス的には？）。

JOINがリレーションでのソートやリレーションの件数の取得にしか使われていないのは、少し意外でした。
### Prismaを使っていて遭遇しそうな課題

検索で指定した条件が、取得結果の型に反映されないことが気になることがありました。例えば、`UserProfile`（to-one側）が存在しないという条件を書いたとしても、取得結果は`UserProfile | null`になります。[^1]

他には個人的な印象ですが、集計のクエリは単純なケースでなければPrismaでは難しいのかなという気もしています。

これらのケースは、[pg-typed](https://github.com/adelsz/pgtyped)や[sqlc](https://sqlc.dev/)（Go）などの、SQLから型を生成するアプローチが向いていそうです。[^2]
...
[^1]: INNER JOINすれば解決できるものの、今度は`user.profile`のようにアクセスできないことが気になりそうです
[^2]: Node.js x MySQLではどちらも使えないので羨ましい…
