...
外部キーに対してインデックスを設定していないと、エディタ上でワーニングが出るため気づきやすいと思います。
...
主キーにはインデックスが付与されるのですが、PrismaのLanguage serverが認識してくれなかったため、余分にインデックスを追加しました。
...
なぜ`prisma migrate`を使わないかというと、PlanetScaleのブランチ機能とフィットしないからです。具体的には、次の2点があります。
